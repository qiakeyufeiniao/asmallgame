...
location=[]
for k in range(M):
    location.append(((SCALE-10)/N*math.cos(-2*math.pi*(k+2)/M)+SCALE/N, \
                     (SCALE-10)/N*math.sin(-2*math.pi*(k+2)/M)+SCALE/N))
...
class wscwithblacktiles():
    ...
    def draw_pointcircle(self, canvas):
        for pt in self.dict_pts:
            canvas.draw_circle(loc(pt), radius, 2, "Green", "White")
            canvas.draw_text(str(pt),  (loc(pt)[0]-10*N/2, loc(pt)[1]+3), textsize, "Red")
    ...
    def draw_pointcircle2(self, canvas):
        for pt in self.dict_pts:
            ...
            canvas.draw_text(str(pt),  (loc2(pt)[0]-10*N/2, loc2(pt)[1]+3), textsize, "Red")
